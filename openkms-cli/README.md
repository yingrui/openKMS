...
**Baidu Cloud (baidu-doc-parse pipeline):** Set **`OPENKMS_BAIDU_CLOUD_API_KEY`** and **`OPENKMS_BAIDU_CLOUD_SECRET_KEY`**. Optional endpoint overrides: **`BAIDU_TOKEN_URL`**, **`BAIDU_TASK_URL`**, **`BAIDU_QUERY_URL`**. With **`--document-id`** (worker pipeline), **`auto`** uses **`file_data`** for files ≤ **5MB**, otherwise a signed **`file_url`** on **`OPENKMS_FRONTEND_URL`**. Override with **`OPENKMS_BAIDU_UPLOAD_MODE`** (`auto` \| `file_data` \| `file_url`). Local **`parse run`** without document id uses **`file_data`** when under Baidu caps (images ≤10MB, other ≤50MB).
