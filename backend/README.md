...
| `OPENKMS_AGENT_MODEL_ID` | (empty) | `api_models.id` for the **embedded** wiki agent (`/api/agent/...`); if empty, the **default** `chat-completions` model in **Models** (set **Set as default** in the UI) is used. |
