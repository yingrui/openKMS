...
User: "50 岁客户买了 <product_code> 重疾险，确诊了严重肺泡蛋白沉积症，能赔吗？"
Output: {"cypher": "MATCH (a:AgeSegment)<-[:targets]-(p:InsuranceProduct {product_code:\\"<CODE>\\"})-[:provides]->(c:Coverage)-[:covers_disease]->(d:Disease) WHERE a.name CONTAINS \\"中老年\\" AND d.disease_name CONTAINS \\"肺泡蛋白沉积\\" RETURN a, p, c, d", "explanation": "Walk from the age segment via the product to its coverage clauses to the matching disease."}
