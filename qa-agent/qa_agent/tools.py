"""LangChain tools for ontology: schema and Cypher. Tools receive access_token via configurable."""
import json
...
def get_ontology_schema_tool(
    ...
    Returns object types with neo4j_label (for MATCH (n:Label)) and link types with neo4j_rel_type, source_neo4j_label, target_neo4j_label."""
    try:
...
def run_cypher_tool(
    ...
    Example: MATCH (d:Disease {name: 'Heart Attack'})-[r:COVERS]-(p:Insurance_Product) RETURN p.name"""
...
def get_ontology_tools() -> list:
    """Return the ontology tools for the agent."""
    return [get_ontology_schema_tool, run_cypher_tool]
