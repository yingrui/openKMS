...
本文从**业务与知识工程**角度说明 openKMS 希望达成的方向；具体能力与实现路径见 [功能索引](functionalities.md)、[架构](architecture.md) 与 [开发计划](development_plan.md)（已交付索引、战略重点与 backlog）。文档入口见 [首页](index.md)。
...
Console、连接器、评估等主要服务 **知识管理员** 及 **法务合规、内控、体系管理** 等岗位；**业务人员与业务专家** 的默认路径是否够短（例如问完能否沉淀为 FAQ/维基段落——见 [开发计划](development_plan.md)）决定系统能否持续有人贡献。
...
| 提质 | 失效未觉、无暇沉淀 | 知道哪里缺、哪里错、改完是否变好 | 评测集与对比运行（→ 改稿与补洞闭环见 [开发计划](development_plan.md)） |
...
| 无征不信 | [智能体知识服务](#goals-agent-service)、评测与出处（[开发计划](development_plan.md)） |
...
**产品缺口：** [连接器](features/connectors.md) — Tushare 同步与智谱搜索已交付；**更多同步种类**及数据集→知识库下游联动仍待完善（[backlog](development_plan.md#connectors-high)）。
...
**方向：** 文档通道、VLM 解析流水线与可编辑 Markdown，使「机器先读、人再兜底」成为常态。图像、音视频等证据型媒体的编目与检索见 [开发计划 — 多模态](development_plan.md#multimodal-models--media-high) 与 [知识类型 — 富媒体](features/knowledge-types.md#rich-media-and-3d)。
