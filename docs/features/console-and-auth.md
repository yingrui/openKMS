...
- **Console overview** (`/console`): Introduces **console sidebar** tools only—**system health**, permissions, data security, data sources, users & feature toggles, settings; cards link when `canAccessPath` allows; quick links to Permissions and Access groups when those permissions apply; optional nudge when the catalog still has only **`all`** (until `openkms_permissions_onboarding_dismissed` is set).
...
- **Data Sources** (`/console/data-sources`): `console:data_sources` (or admin). **Connectors** (`/connectors`, detail **`/connectors/:id`**): **`connectors:read`** lists kinds and configured instances; **`connectors:write`** creates (modal on list), updates, or deletes on the detail page. Kind **`category`** is **`sync`** (dataset **outputs** for future sync jobs, e.g. Tushare) or **`search_tool`** (no instance outputs; kind **`output_schema`** describes search responses, e.g. Zhipu web search). **sync** detail pages (e.g. Tushare) use **General**, **Output datasets**, **Schedule**, and **Probe** tabs (credentials and settings on **General**; dataset slot bindings on **Output datasets**; **Schedule** stores cron metadata (write-through to `scheduled_triggers`) and supports **Run sync now**; the **scheduler** process runs enabled schedules; **Probe** sends live Tushare `daily` requests with JSON response, no dataset writes). **search_tool** detail pages use **General** and **Playground** tabs (editable request parameters, Send, JSON response). **search_tool** connectors expose **`POST /api/connectors/{id}/search`** and can be bound in **Agents** project settings (`web_search`, `search_connector_id`) for the `web_search` tool. Shared fields: **inputs**, optional **extra settings**, **encrypted secrets**. **Datasets** (`/ontology/datasets`): **`console:datasets`** to register and manage; **`ontology:read`** to view (filtered by dataset ACL). **Object types / link types** (`/ontology/object-types`, `/ontology/link-types`): `console:object_types` / `console:link_types` **or** `ontology:read` / `ontology:write` as applicable; System Settings, Users & Roles, Feature Toggles remain `console:*` (or admin).
...
- Feature toggles: **`evaluations`** (default off), **`connectors`** (default on), and **`agents`** (default on) – persisted in PostgreSQL (`feature_toggles` table), shared across all users/devices. When **`agents`** is off, the SPA hides `/agents` and `/projects/*`, and `/api/projects/*` plus `/api/user/git-credentials` return **404**.
