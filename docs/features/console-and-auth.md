...
- **Data Sources** (`/console/data-sources`): `console:data_sources` (or admin). **Connectors** (`/connectors`, detail **`/connectors/:id`**): **`connectors:read`** lists kinds and configured instances; **`connectors:write`** creates (modal on list), updates, or deletes on the detail page—**inputs** (kind-defined, e.g. API base URL), **outputs** (slot → ontology **dataset** for sync jobs), optional **extra settings** (JSON key–value), and **encrypted secrets** (same Fernet key as data source passwords; e.g. `TUSHARE_TOKEN` for kind `tushare`). **Datasets** (`/ontology/datasets`): **`console:datasets`** to register and manage; **`ontology:read`** to view (filtered by dataset ACL). **Object types / link types** (`/ontology/object-types`, `/ontology/link-types`): `console:object_types` / `console:link_types` **or** `ontology:read` / `ontology:write` as applicable; System Settings, Users & Roles, Feature Toggles remain `console:*` (or admin).
