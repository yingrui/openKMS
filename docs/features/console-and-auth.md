...
- **Data Sources** (`/console/data-sources`): `console:data_sources` (or admin). **Connectors** (`/connectors`): **`connectors:read`** lists kinds and configured instances; **`connectors:write`** creates, updates, or deletes integrations—**inputs** (kind-defined, e.g. API base URL), **outputs** (slot → ontology **dataset** for sync jobs), optional **extra settings** (JSON key–value), and **encrypted secrets** (same Fernet key as data source passwords; e.g. `TUSHARE_TOKEN` for kind `tushare`). **Datasets and schema** (`/ontology/datasets`, `/ontology/object-types`, `/ontology/link-types`): `console:datasets` / `console:object_types` / `console:link_types` **or** `ontology:read` / `ontology:write` as applicable (API uses `require_any_permission`); System Settings, Users & Roles, Feature Toggles remain `console:*` (or admin).
