...
- **Entry**: header user menu **Console** (when outside console routes) opens `/console` for users with the **`admin`** role, permission **`all`**, or **`console:access`**. Inside `/console/*`, the header shows **Exit Console** back to home (`/`). The console shell and route gate still use `canAccessPath` against `frontend_route_patterns` from `GET /api/auth/permission-catalog` for individual pages (plus feature toggles where applicable).
...
- **Data Sources** (`/console/data-sources`): `console:data_sources` (or admin). **Object storage** (`/console/storage`): `console:storage` — paginated bucket browser (S3 `list_objects_v2`); create folder placeholders; multi-select move prefixes/objects; metadata only (no presigned download). **Connectors** (`/connectors`): **`connectors:read`** / **`connectors:write`** — see [Connectors](connectors.md). **Datasets** (`/ontology-manager/datasets`): **`console:datasets`** to register and manage; **`ontology:read`** to view (filtered by dataset ACL). **Object types / link types** (`/ontology-manager/object-types`, `/ontology-manager/link-types`): `console:object_types` / `console:link_types` **or** `ontology:read` / `ontology:write` as applicable; System Settings, Users & Roles, Feature Toggles remain `console:*` (or admin).
