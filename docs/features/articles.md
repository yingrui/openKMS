...
| Article content review | ✅ | **`POST /api/articles/{id}/review`** — rubric-based LLM judge on article markdown using channel review settings; results in **`article_reviews`**; **`GET …/reviews`**, **`GET …/reviews/latest`**; SPA **Article detail → Content review** |
