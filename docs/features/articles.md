...
|---------|--------|-------------|
| Article channels | ✅ | Tree CRUD **`GET/POST/PUT/DELETE /api/article-channels`** (merge, reorder); no document-style parsing pipeline on channels; per-channel **LLM review** config (`review_model_id`, `review_prompt`, `review_criteria`) on **`PUT /api/article-channels/{id}`** and SPA **Channel settings → Review** |
| Article content review | ✅ | **`POST /api/articles/{id}/review`** — rubric-based LLM judge (pydantic-ai structured output) on article markdown using channel review settings; results in **`article_reviews`**; **`GET …/reviews`**, **`GET …/reviews/latest`**; SPA **Article detail** auto-loads latest review on open (pass/fail badge on collapsed **Content review** header) |
...
| openkms-skill (HTTP) | ✅ | **`articles relationships list`**, **`create`**, **`delete`** (personal API key; same outgoing-only delete rule as documents); see [openkms-skill](openkms-skill.md) |
...
| SPA | ✅ | **`ArticleChannelsContext`** + sidebar tree; hub **`/articles`**; **`/articles/channels`** manage tree; **`/articles/channels/:id`** list with row checkboxes, per-row **Move** / **Delete**, and bulk **Move selected** / **Delete selected** (sequential **`PATCH`** / **`DELETE`** per article); **New article** modal: title required, optional source ID/URL + body; **`/articles/channels/:id/settings`** (tabs: General, **Review**, Sharing); **`ArticleDetail`** **Article information** card holds title + source **Edit** + Save/Cancel (**`PATCH /api/articles/:id`**), channel/lifecycle/applicable/updated, collapsible **Relationships**, collapsible **Content review** (**Run review**, per-criterion scores, suggestions), and collapsible **Attachments** (count badge, **Add file**, **Insert link**, **Remove**); markdown read mode includes a **Print** action that opens a print-only markdown layout in the browser print dialog; markdown editor below has **Image** / **Attachment** toolbar buttons + paste/drag-drop upload + Save (**`PUT …/markdown`**); **Delete article** at the bottom of the info card; legacy **`/articles?channel=`** → channel route |
...
`$API` is your backend base URL (e.g. `http://localhost:8102`); for `$TOKEN` see [Obtaining an API token](console-and-auth.md#obtaining-an-api-token).
