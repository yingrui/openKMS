...
| POST | `/api/connectors/{id}/sync` | Queue a **sync** connector job (`run_connector_sync`); returns `{ job_id }` (202). Body optional: `{ start_date, end_date }` (ISO dates, inclusive). **Manual:** pass both dates. **No dates:** each connector kind resolves its own default window (Tushare: incremental daily from last row through today). **Tushare** output slots: `trade_calendar`, `stock_basic` (one `stock_basic` API call per sync — listed stocks, `list_status=L`), `stock_trade_daily`; respects `settings.sync_api_min_interval_seconds` and `sync_trade_cal_min_interval_seconds` (default **0.31** s ≈ 200 req/min); skips `trade_cal` when the calendar table already spans the window; on rate limits, short waits retry in-process and longer waits re-queue with `scheduled_at` |
