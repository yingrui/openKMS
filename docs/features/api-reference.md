...
| GET | `/api/app-builder/apps` | List apps (`?status=published`); `ontology:read` — same handlers as legacy `/api/ontology/apps` |
| POST | `/api/app-builder/apps` | Create app (name + api_name; optional bindings; stub A2UI if unbound) (`ontology:write`) |
| GET | `/api/app-builder/apps/{id}` | Published run document only (404 if draft) |
| GET | `/api/app-builder/apps/{id}/design` | Draft A2UI for App Builder |
| PATCH | `/api/app-builder/apps/{id}` | Update metadata / bindings / draft |
| DELETE | `/api/app-builder/apps/{id}` | Delete app |
| POST | `/api/app-builder/apps/{id}/synthesize` | Reset draft from bindings |
| POST | `/api/app-builder/apps/{id}/publish` | Publish draft (optional `a2ui_messages`) |
...
| PATCH/DELETE | `/api/projects/{id}/conversations/{cid}` | Update title / delete session (**`409`** while a non-stale `last_turn.status=running`; delete clears LangGraph checkpoint thread) |
| POST | `/api/projects/{id}/conversations/{cid}/suggest-title` | LLM title from message history |
| GET/POST | `/api/projects/{id}/conversations/{cid}/messages` | **GET** history: query **`limit`**, **`offset`**, optional **`tail=true`** (last `limit` messages; ignores offset) → `{ items, total, limit, offset }`. **POST** body: `content`, optional `stream`, optional `session_id` (Langfuse), optional `mode` (`plan` \| agent). **`stream: true`** → NDJSON (`user`, `delta`, `tool_*`, `subagent_*`, `todo`, `interrupt`, **`error`**, `done`). Interactive streams **early-commit** user + empty assistant (`last_turn=running`) and run the agent in a **background task** (client disconnect cancels NDJSON only). **`409`** if a non-stale turn is already `running`. On failure: persist assistant row + **`error`** `{ detail, message }` (HTTP **200**). Streamed assistant rows persist `tool_calls` (`wiki_tool_traces_v1`) for UI replay; **`context.last_turn`** updated on the conversation |
| DELETE | `/api/projects/{id}/conversations/{cid}/messages/from/{message_id}` | Delete this message and everything after it; clears LangGraph checkpoint thread (revert / regenerate). **`409`** while a non-stale turn is `running` |
| POST | `/api/projects/{id}/conversations/{cid}/messages/resume` | HITL resume (`decision`: approve/reject/edit/respond); same durable stream contract as POST messages; **`409`** if already `running` |
