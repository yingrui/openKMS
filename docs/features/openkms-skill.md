...
Domain ontologies (e.g. Tushare → Stock → read-only Functions) are **tenant DIY** via `SKILL.md` Workflow **G** — not platform seeds. Action execute does not yet apply durable object writes; see [Manager alignment](../research/ontology_manager_alignment.md).
