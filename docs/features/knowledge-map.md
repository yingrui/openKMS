...
| Knowledge Map UI | ✅ | **`/knowledge-map`** (lazy; **`/taxonomy`** redirects); sidebar **Knowledge Map** above **Glossaries** when feature toggle + path allowed; sitemap-style copy; **Edit map** vs **Explore (3D)** tabs: tree + **Node details** (refer-tos, CRUD) on Edit; optional **`KnowledgeMapForceGraph3D`** (`react-force-graph-3d`, lazy chunk) for orbit/pan/zoom exploration—taxonomy selection syncs to tree + `?node=`; **New node** modal; reorder/move/edit/delete |
