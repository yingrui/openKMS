...
| QA Agent service | ✅ | Separate FastAPI + LangGraph project (`qa-agent/`); RAG via backend search API; hybrid recall then optional cross-encoder rerank (`POST …/v1/rerank`). If the LLM host returns **404** on rerank, set **`OPENKMS_RERANK_BASE_URL`** to a provider that implements rerank or **`OPENKMS_RERANK_ENABLED=false`**. Langfuse: bearer token is not copied into trace metadata; **`LANGFUSE_TRACE_STREAMING`** (default false) enables Langfuse on **`/ask/stream`** (may log OpenTelemetry context warnings). Skills include ontology and page_index |
| Q&A | ✅ | **Q&A** button (top right when an agent URL is set) opens **full-page** chat; answers **stream** over **`POST …/ask/stream`** (NDJSON: model `delta`, tool lifecycle events, then `done`; assistant text rendered as **markdown**; tool calls shown like wiki copilot). **Back** returns to tabs. Without an agent URL, the button is hidden |
