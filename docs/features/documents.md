...
| Document channel view | ✅ | Browse documents by channel at `/documents/channels/:channelId`; list from `GET /api/documents?channel_id=`; multi-select with bulk **Process**, **Move**, and **Delete** |
...
| Document processing | ✅ | Process button on list/detail → `POST /api/jobs` (optional **`force_reparse`**; Job runs UI checkbox). **XLSX** runs use `run_spreadsheet_preview`; **XMIND** runs use `run_mindmap_preview` (no channel pipeline required). Other types: **`force_reparse` false** reuses `{file_hash}/result.json` on storage when present (fast path after reset + re-process same file). **`force_reparse` true** or **Retry** on a failed run always runs the VLM CLI again. Channel auto-process uses `force_reparse=false` |
...
- **Baidu Cloud PaddleOCR-VL API** (`baidu-doc-parse` pipeline / `parse run --method baidu-doc-parse`) — **`file_url`** (default when pipeline passes `--document-id`): backend mints a signed URL on **`OPENKMS_FRONTEND_URL`** (`GET /api/public/documents/{id}/original.{ext}`) for Baidu to fetch; **`file_data`** (base64) for local `parse run` or **`OPENKMS_BAIDU_UPLOAD_MODE=file_data`**; credentials **`OPENKMS_BAIDU_CLOUD_*`** and optional **`BAIDU_*_URL`** in **`openkms-cli/.env`**; page preview PNGs and bbox coords use **2× `page_meta`** (same grid as local VLM)
