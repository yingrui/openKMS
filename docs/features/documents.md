...
| Document channel view | ✅ | Browse documents by channel at `/documents/channels/:channelId`; list from `GET /api/documents?channel_id=` with **pagination** (offset/limit, page size 10–100); multi-select on the current page with bulk **Process**, **Move**, and **Delete** |
...
| Document processing | ✅ | Process button on list/detail → `POST /api/jobs` (optional **`force_reparse`**; Job runs UI checkbox). **Disabled with tooltip** when the channel has no default pipeline (PDF/images/Office); **XLSX** / **XMIND** do not require a pipeline. **XLSX** runs use `run_spreadsheet_preview`; **XMIND** runs use `run_mindmap_preview`. Other types: **`force_reparse` false** reuses `{file_hash}/result.json` on storage when present (fast path after reset + re-process same file). **`force_reparse` true** or **Retry** on a failed run always runs the VLM CLI again. Channel auto-process uses `force_reparse=false` |
...
- **Metadata extraction**: when channel has extraction_model_id and extraction_schema, worker passes `--extract-metadata --extraction-model-name <model_name>`; CLI fetches model config from `GET /internal-api/models/config-by-name`, extracts via pydantic-ai **PromptedOutput(StructuredDict)** with `response_format: json_object` (DeepSeek-compatible), PUTs to `PUT /api/documents/{id}/metadata`; **LLM failure does not fail the pipeline** after a successful parse
