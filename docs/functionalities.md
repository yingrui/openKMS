...
| Document detail | ✅ | View parsed Markdown at `/documents/view/:id`; right panel: Markdown | Page Index toggle (PageIndex tree from pipeline; refresh icon on Page Index tab parses markdown to tree via `POST /rebuild-page-index`; content icon per node opens popup with section content including children); scrollable layout (min-height 720px) |
