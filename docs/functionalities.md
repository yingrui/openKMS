...
| Wiki files | ✅ | `POST .../files` (multipart), `GET .../files/{id}/content` (presigned redirect), `DELETE .../files/{id}`; S3/MinIO uses **`wiki/{space_id}/vault/{relative-path}`** when the multipart filename is a valid vault-relative path (folder import + CLI), else `wiki/{space_id}/files/{file_id}/…` |
...
| UI | ✅ | `/wikis`, `/wikis/:id`, `/wikis/:id/pages/:pageId`; wiki space detail: paginated **Pages** list (25 per page), **Import folder** / **Import zip**; sidebar **Wikis** when toggle + path allowed; preview uses `react-markdown` with `/api/...` image URL resolution |
