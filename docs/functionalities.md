...
| Wiki assistant API | ✅ | `POST/GET` **`/api/agent/conversations`** (list by `wiki_space_id` + `surface`), **`GET`/`DELETE`/`PATCH` `/api/agent/conversations/{id}`**, **`/api/agent/conversations/{id}/messages`**: `POST` with `stream: true` returns NDJSON (token streaming); `stream: false` returns JSON; first user message can set **title** when empty — see [wiki_agent_prototype.md](./wiki_agent_prototype.md) |
