...
| Home hub | ✅ | Signed-in `/` loads `GET /api/home/hub` when **taxonomy:read** or **documents:read** (hub JSON: taxonomy counts, work items, placeholder **share_requests**). With **taxonomy:read** (and `taxonomy` toggle): **Knowledge Map mind map** is the page center (`KnowledgeMapMindMap`: `GET /api/taxonomy/nodes/tree` + `GET /api/taxonomy/resource-links`, channel/wiki labels); clicking a node opens **`/knowledge-map?node=`**; work items and browse shortcuts sit below. Without taxonomy read: work items + shortcuts only (centered column) |
