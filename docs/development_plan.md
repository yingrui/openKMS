...
- Route protection: home public; other pages show "Authentication Required" when not logged in; **`401`** responses whose body indicates invalid/expired JWT clear SPA session via **`authAwareFetch`** / **`AuthContext`** so the same gate appears instead of raw API error JSON
