...
- **Document lifecycle & lineage**: `series_id`, `effective_from` / `effective_to`, `lifecycle_status` on documents; `document_relationships` (supersedes, amends, repeals, implements, see_also); `PATCH /lifecycle`, `GET/POST/DELETE /relationships`; `is_current_for_rag` on document API; default KB semantic search and kb-index (`lifecycle_index_mode` default `current_only`) use only current documents unless opted out; document detail **Lineage & lifecycle** UI collapsed by default (expand button loads relationships)
