...
- **Knowledge Map & home hub**: `taxonomy_nodes` / `taxonomy_resource_links`; **`GET /api/taxonomy/nodes/tree`**, node PATCH (move/reorder/edit) + link CRUD; **`GET /api/home/hub`** (taxonomy summary in JSON + scoped document relationship work items + placeholder share requests); SPA **Knowledge Map** at **`/knowledge-map`** (legacy **`/taxonomy`** redirects; sidebar above Glossaries; **Tree** + **Node details** panels with scoped refer-tos; **New term** modal); permissions **`taxonomy:read`** / **`taxonomy:write`**; feature toggle key **`taxonomy`** (Console label: Knowledge Map)
...
- **Wiki spaces**: `wiki_spaces`, `wiki_pages`, `wiki_files` (+ `access_group_wiki_spaces`); API `/api/wiki-spaces` (scoped like KBs when `OPENKMS_ENFORCE_GROUP_DATA_SCOPES`); PageIndex cache on page body; **`GET /api/wiki-spaces/{id}/graph`** Graph View data (wikilinks + relative markdown links; optional S3 cache `wiki/{space_id}/link-graph.json` vs `max(updated_at)`); file uploads + presigned GET; S3/MinIO **vault mirror** prefix `wiki/{space_id}/vault/{relative-path}` for imports (binaries + `.md` bodies) and vault-style multipart filenames; **POST `/api/wiki-spaces/{id}/import/vault`** (folder multipart or zip) for Obsidian-style bulk import; permissions `wikis:read` / `wikis:write`; feature toggle `wikiSpaces`; UI `/wikis`, **`/wikis/:id/graph`** (Graph View), editor, vault import on space detail (**folder**: in-app modal with skip options, then folder picker; after the browser’s upload confirmation, import starts immediately—no extra in-app confirm); **paginated** wiki page list on space detail (15 per page in the SPA); **openkms-cli** `wiki put`, `wiki sync`, `wiki upload-file`; **wiki assistant + Documents tab UI prototype** on space detail ([wiki_agent_prototype.md](./wiki_agent_prototype.md) — sessionStorage links + local chat until backend agent and `wiki_space_documents` land)
...
- [x] UI prototype: wiki space detail — **Pages \| Documents** tabs, linked-docs picker (`GET /api/documents`), **Wiki assistant** panel (local-only messages)
- [ ] Alembic + API: `wiki_space_documents`, link/unlink/list; replace sessionStorage in the SPA
- [ ] Backend embedded agent: `/api/agent`, LangGraph wiki graph, Langfuse (optional), conversation persistence — see design doc backlog
