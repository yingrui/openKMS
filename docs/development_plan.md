...
- **Taxonomy & home hub**: `taxonomy_nodes` / `taxonomy_resource_links`; **`GET /api/taxonomy/nodes/tree`**, node PATCH (move/reorder/edit) + link CRUD; **`GET /api/home/hub`** (taxonomy summary + scoped document relationship work items + placeholder share requests); SPA **Taxonomy** at `/taxonomy` (sidebar above Glossaries; terms tree + term-detail panel with scoped refer-tos; **New term** modal); permissions **`taxonomy:read`** / **`taxonomy:write`**; feature toggle **`taxonomy`**
