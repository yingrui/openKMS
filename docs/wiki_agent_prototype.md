...
| **agent_messages** | `role` (`user` \| `assistant` \| `tool` reserved), `content` (user-visible assistant text), `tool_calls?` JSONB — wiki Copilot stores **`wiki_tool_traces_v1`** (tool name + output) for **model replay** on later turns and optional **`wiki_assistant_stream_parts_v1`** for interleaved tool/text UI replay after reload. KB Q&A may add **`kb_qa_sources_v1`** (and optional **`wiki_tool_traces_v1`** / **`wiki_assistant_stream_parts_v1`** when the qa-agent streams tool lines) on assistant rows for persisted references + tool rail after reload. |
...
4. **Reloaded threads + tool rail** — `GET /api/agent/conversations/{id}/messages` returns `tool_calls` including **`wiki_tool_traces_v1`** and, when streamed, **`wiki_assistant_stream_parts_v1`** (interleaved tool/text order). Rebuild assistant `streamParts` in `frontend/src/components/wiki/wikiCopilotStreamParts.ts` (prefer persisted interleaved parts; fallback: completed tools then `content`). Keep keys aligned between `backend/app/services/agent/assistant_stream_parts.py`, `wiki_runner.py`, and `wikiCopilotStreamParts.ts`.
